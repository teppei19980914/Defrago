...
| ユーザー数 | gtd_items | notifications | trash | 合計推定サイズ |
|-----------|-----------|---------------|-------|-------------|
| 10人 | 1,000行 | 100行 | 100行 | ~1 MB |
| 100人 | 10,000行 | 1,000行 | 1,000行 | ~10 MB |
| 1,000人 | 100,000行 | 10,000行 | 10,000行 | ~100 MB |
| 10,000人 | 1,000,000行 | 100,000行 | 100,000行 | ~500 MB (上限に近い) |

**Neon Free上限 0.5GB到達**: 約10,000ユーザー（Neon Launchプランへの移行: $19/月）

**ゴミ箱の自動削除**: アプリ起動時に30日経過したアイテムを物理削除するため、無制限に肥大化することはない。
...
| 新規ユーザー登録の一時停止 | 既存ユーザーのみ利用可（labels.jsonで制御中） | - |
...
| 日付 | 変更内容 |
|------|---------|
| 2026-04-07 | v2.0.0 エッセンシャル思考に基づく大改修（4分類ボタン化、整理フェーズ削除、ゴミ箱機能追加） |
